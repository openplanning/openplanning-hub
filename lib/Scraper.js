    ...
    async.forever(
        ...
        function(err) {
          if (err.code === 'ECONNRESET' || err.code === 'ENETDOWN' || err.code === 'ENOTFOUND') {
            ...
            setTimeout(_.bind(self.collect, self), (2 * (60* 1000)));
          } else if (err.code === 'ETIMEDOUT' || err.code === 'ESOCKETTIMEDOUT') {
            logger.warn('Request timeout, trying again in 5 mins.');
            setTimeout(_.bind(self.collect, self), (5 * (60* 1000)));
          } else if (err.code === 'INVALID_STATUS_CODE' && self.retries < 5) {
            logger.warn('Invalid http status code, trying again in 5 mins.');
            setTimeout(_.bind(self.collect, self), (5 * (60* 1000)));
          } else if (err) {
            logger.error(err);
            process.exit(1);
